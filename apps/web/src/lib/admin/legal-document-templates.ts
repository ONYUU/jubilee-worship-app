...
3. 알림 이용 시 처리하는 정보
사용자가 별도 고지를 읽고 동의한 뒤 알림을 켜면 무작위 설치 식별자, Expo 푸시 토큰, 플랫폼(iOS 또는 Android), 앱 버전·앱 구분, 선택한 알림 종류, 동의 버전·서버 동의 시각, 만 14세 이상 확인 여부·서버 확인 시각, 최종 연결 시각, 발송·오류 상태를 처리합니다.
...
Supabase 처리: 이 프로젝트의 주 데이터베이스는 대한민국 서울(ap-northeast-2)에 있습니다. Data API와 데이터베이스는 무작위 설치 식별자, 재사용할 수 없도록 해시한 설치 검증값, Expo 푸시 토큰과 해시, 플랫폼·앱 버전·앱 구분, 알림 선택, 동의 버전·동의 시각, 만 14세 이상 확인 정보, 최종 연결 시각, 발송·오류 상태와 보안·API 요청 메타데이터를 처리할 수 있습니다. 확인한 공식 DPA 문서에서 계약 주체로 표시된 Supabase, Inc.와 지원 서비스 재수탁자로 표시된 SUPABASE PTE. LTD.를 구분하며, 실제 계정에 적용되는 최신 계약·위탁 구조를 공개 전에 확정합니다.
...
Apple·Google 처리: Expo의 2026년 8월 17일자 공식 하위처리자 목록은 Apple과 Google의 푸시 관련 처리 국가를 미국으로 표시합니다. 기기 푸시 토큰, 알림 제목·본문·딥링크, 전달 메타데이터는 알림을 기기로 전달하기 위해 처리될 수 있습니다.
