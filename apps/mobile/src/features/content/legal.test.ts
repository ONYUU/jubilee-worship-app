...
Expo는 운영체제 기기 푸시 토큰(APNs 또는 FCM)과 Expo 앱 설치 식별자를 사용해 Expo 푸시 토큰을 발급·갱신합니다. 실제 알림 발송 때 Expo는 알림 제목·본문·딥링크와 티켓·영수증·오류를 처리하고, Apple 또는 Google은 운영체제 기기 푸시 토큰을 처리합니다. Supabase는 대상 종류·관련 예배 일정, 발송 승인·대기 상태와 설치별 발송 시도를 처리합니다. 철회 시 Expo의 운영체제 토큰 자동 갱신을 끄고 APNs 또는 FCM 기기 토큰 등록 해제를 요청하며, 실패하면 보안저장소에 정리 대기 상태를 남깁니다.
