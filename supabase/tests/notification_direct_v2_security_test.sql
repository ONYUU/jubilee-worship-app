...
select is(
  ...
  'the database requires the exact v5 sensitive-interest consent version'
);
select is(
  private.current_sensitive_interest_disclosure_sha256(),
  '575ecb39ce1c1670e169e5fdae28587b09477a765a80c6dcfdb5df2f170a5f0e',
  'the database pins the exact v5 Korean disclosure digest'
);
...
select is(
  public.notification_register_v2(
    'a0500000-0000-4000-8000-000000000000',
    private.sha256_hex(repeat('0', 64)),
  ...
  '23514',
  'the database rejects a stale v4 consent version after the v5 migration'
);
delete from private.notification_client_rate_limits;

-- An expected failure is typed, and its source counter commits.
select is(
  public.notification_register_v2(
    ...
    'android', '0.1.0+1', 'production',
    'sensitive-interest-notifications-v5', repeat('0', 64), 'ko-KR',
    true,
    true, false, false
  ) ->> 'code',
  '23514',
