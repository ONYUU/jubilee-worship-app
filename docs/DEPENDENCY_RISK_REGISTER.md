...
- 현재 상태: GitHub Advisory는 `2.0.3` 이상을 패치 버전으로 안내하지만, 2026-08-15 npm 레지스트리의 최신 버전은 `2.0.2`이며 `2.0.3`은 배포되지 않음
- 적용 완화:
...
- 적용 완화: 애플리케이션 코드에서 해당 API를 호출하지 않으며, Expo 공식 호환 패치가 나오기 전 major override를 적용하지 않음
