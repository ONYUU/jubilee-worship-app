...
- 대상 플랫폼: iOS, Android
- 현재 단계: 앱·웹 관리자·데이터베이스 핵심 기능 구현, 출시 연동·실기기 검증 진행 중
- 확정 디자인: `Rose Haze · Logo-free v6`
- 기준 목업: [앱 디자인 전체 보드](../design/mobile/output/app-design-board-v6-rose-logo-free-sermon.png)
...
4. 일정 변경·취소, 송리스트 공개·변경, 선택한 예배 하루 전 알림을 제공한다.
...
|---|---|---|
| 앱 디자인 | 로고 없는 v6 목업 6화면 확정, 홈·예배에 설교 정보 포함 | 실제 네이티브 화면과 상호작용 구현 |
...
|---|---|---|---|
| 시작 화면 | 로즈베이지 배경과 텍스트 표시 | 앱 이름은 고정, 배경은 앱 배포 자산 | 신규 앱 구현 |
...
| 기능 | 동작 원칙 |
|---|---|
| 캘린더 추가 | 시스템 일정 추가 화면을 열고 사용자가 직접 저장을 확정한다. 앱은 캘린더 읽기 권한을 요청하지 않는다. |
...
- 알림 동의 시 무작위 설치 ID, 기기 푸시 토큰, 플랫폼·앱 버전과 선택 설정만 저장하고 개인정보 처리방침에 목적·보관·삭제 절차를 명시한다.
...
- 문자·그래픽 로고가 없는 불투명 `Rose Haze` 설치 아이콘 원본과 Android 적응형 아이콘 안전영역
