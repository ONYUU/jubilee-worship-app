...
캘린더 버튼은 시스템 일정 작성 화면을 엽니다. iOS 16에서는 일정 추가 전 캘린더 권한을 요청하지만 앱은 기존 일정을 조회하거나 서버로 전송하지 않습니다. iOS 17 이상과 Android에서는 이 권한을 요청하지 않습니다. 길찾기, YouTube, Instagram 링크는 사용자가 선택한 경우에만 외부 앱 또는 웹을 엽니다.
```
