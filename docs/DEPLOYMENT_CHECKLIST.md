...
- [x] 지원 문의를 Google Workspace 업무용 계정으로 전환하고 문의 해결일 또는 마지막 답변일로부터 90일 후 삭제하는 제품 기준 승인
- [ ] 실제 Google Workspace 도메인·주소·관리자·계약/CDPA 설정 및 삭제 운영 증거 확정
- [x] 알림 기능만 만 14세 이상으로 제한하고 별도 자기확인 gate 구현; 공개 콘텐츠는 연령과 관계없이 열람 가능
...
- [x] 현재 커밋 `aaeed5e` EAS Preview build `f60bcbcc-de16-457b-99cd-d3a9460df37d`를 Samsung SM-G991N에 replace install하고 공식 아이콘·새 홈 사진·4개 탭·테마 유지·3종 Back·만 14세 gate·crash/ANR 0건 확인
...
- [x] Supabase remote migration 15/15을 로컬과 일치하도록 적용
...
2. [x] migration 15/15·Edge Function 8개 적용, legacy endpoint 410, 등록 중단·미생성·Supabase custom-header log canary 확인
3. [ ] `cf-connecting-ip` injection/spoof, gateway/WAF, 사용량·차단량 alert, 일일 101번째·전체 501번째 및 25시간 만료 원격 검증
4. [ ] 법적 처리자·담당자·연락처·Google Workspace·만 14세 절차·국외 처리 근거를 확정하고 검토 완료 정책 공개
5. [ ] `TEST_PUSH_PAIRING_PEPPER`를 서버 secret으로 설정하고 최초 owner·development/preview 실기기 pairing 검증
6. [ ] 승인된 점검 창에서만 등록을 일시 활성화해 실제 기기 token 등록을 확인한 뒤 즉시 다시 비활성화
7. [ ] 사용자 별도 승인 후에만 실제 외부 push·ticket·receipt·`DeviceNotRegistered`를 검증
...
- [x] 현재 checkout의 알림 migration·Edge Function·앱 호출 변경을 Supabase remote에 배포하고 fail-closed smoke test 완료
